...
router.get("/category", async (req, res) => {
	...
	var sql =
		'SELECT a.*, DATE_FORMAT(create_date,"%Y-%m-%d %T") AS create_time , DATE_FORMAT(update_date,"%Y-%m-%d %T") AS update_time, c.`cate_name` AS name FROM `article` , t.`tag_name` AS name FROM `article`  a LEFT JOIN article_category c AND article_tag t ON a.cate_id = c.id AND a.tag_id=t.id ORDER BY create_date DESC, update_date DESC LIMIT ? OFFSET ? WHERE cate_id = ?';
